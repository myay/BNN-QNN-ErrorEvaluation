...
from Test_Models import VGG3_Test_Q
...
def main():
    ...
    nn_model = VGG3_Test_Q

    tensor = torch.rand(size=(2,2,3,3), dtype=torch.float).cuda()

    model = nn_model(cel_train, cel_test, weightBits=Quantization2(quantization.quantize, args.weight, 1), inputBits=Quantization2(quantization.quantize, args.input, 1), quantize_train=q_train, quantize_eval=q_eval).to(device)
